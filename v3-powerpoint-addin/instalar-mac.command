#!/bin/bash
# Instalador do Doodle Studio para PowerPoint (Mac).
...
if curl -fsSL "$URL" -o "$WEF/doodle-studio.xml"; then
  ...
  osascript -e 'display dialog "Doodle Studio instalado! ✅

Abra o PowerPoint e vá em:
Inserir → Add-ins → Developer Add-ins → Doodle Studio." buttons {"OK"} default button "OK" with title "Doodle Studio"' >/dev/null 2>&1
  echo "Instalado em: $WEF/doodle-studio.xml"
else
  osascript -e 'display dialog "Não foi possível baixar o suplemento. Verifique sua conexão e tente de novo." buttons {"OK"} with title "Doodle Studio" with icon stop' >/dev/null 2>&1
