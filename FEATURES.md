...
| **Entrelinha (0.8x/0.9x…)** | **Não é setável via Office.js** (confirmado até preview). Via **macro VBA** `assets/BG-Tipografia.bas` (painel: "Entrelinha — macro"): `BG_AplicarEntrelinha` lê o tamanho da seleção e aplica a entrelinha B+G (`LineRuleWithin`/`SpaceWithin`); `BG_InserirBloco` cria uma caixa já 100% no estilo. Roda por Option+F8 ou botão no QAT. **Alternativa sem macro:** âncora `insertStylesReference()` + Pincel de Formatação |
